...
function beforeParen(label = '') { return String(label).split('(')[0].trim(); }
...
export default function InventoryOperationsPage() {
  ...
  useEffect(() => {
    ...
    if (q.length < 2) { setProductOpts([]); return; }
    (async () => {
      try {
  ...
  useEffect(() => {
    ...
    if (q.length < 2) { setUserOpts([]); return; }
    (async () => {
      try {
  ...
  const clearDates = () => { setPage(1); setStartDate(''); setEndDate(''); };
  const resetFilters = () => {
    setPage(1); setLimit(10); setOperationType('');
    setProductInput(''); setProductId('');
  ...
  const exportCsvAll = async () => { try { await downloadOperationsCSV(exportFilters); } catch (e) { console.error(e); toast.error(e?.message || 'Błąd eksportu CSV'); } };
  const exportPdfAll = async () => { try { await downloadOperationsPDF(exportFilters); } catch (e) { console.error(e); toast.error(e?.message || 'Błąd eksportu PDF'); } };
  ...
  return (
    <div style={{ padding: '2rem' }}>
      ...
      >
        <input list="op-product-options" placeholder="Produkt (nazwa / SKU)" value={productInput} onChange={(e) => onProductChange(e.target.value)} />
        ...
        <select value={operationType} onChange={(e) => { setPage(1); setOperationType(e.target.value); }}>
          ...
          <option value="receive">Odbiór z tranzytu</option>
        ...
        <button onClick={() => { setPage(1); setStartDate(''); setEndDate(''); }}>Wyczyść daty</button>
      ...
      <div style={{ marginBottom: 8, display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <button onClick={exportCsvAll}>Eksport CSV (filtry)</button>
        <button onClick={exportPdfAll}>Eksport PDF (filtry)</button>
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {ops.length === 0 && !loading && (
              <tr><td colSpan={7} style={{ textAlign: 'center', padding: 16 }}>Nic nie znaleziono.</td></tr>
            )}
            {ops.map(op => (
              <tr key={op.id} style={rowStyle.body}>
                ...
                <td style={cellStyle.body}>{op.user ? `${op.user.firstName || ''} ${op.user.lastName || ''}`.trim() || op.user.username : '—'}</td>
