...
router.get('/', async (req, res) => {
	try {
		...
		if (minPrice)
			whereClause.price = {
				[require('sequelize').Op.gte]: parseFloat(minPrice),
			};
		if (maxPrice) {
		...
		const productsWithStock = products.rows.map((product) => {
			...
			const totalStock = productData.inventoryItems.reduce(
				(sum, item) => sum + item.quantity,
				0
			);
			const totalReserved = productData.inventoryItems.reduce(
				(sum, item) => sum + item.reservedQuantity,
				0
			);
			const totalAvailable = productData.inventoryItems.reduce(
...
router.put('/:id', async (req, res) => {
	try {
		...
		if (updateData.maxStockLevel)
			updateData.maxStockLevel = parseInt(updateData.maxStockLevel);
